...
struct UsagePressureView: View {
    ...
    private var pressure: Double? {
        guard let sevenDay = usage.sevenDay,
              let resetsAt = sevenDay.resetsAtDate else { return nil }
        ...
        guard secondsElapsed > 0, secondsRemaining > 0 else { return nil }
    ...
    private var statusLabel: String {
        guard let p = pressure else { return "Unknown" }
        let abs = abs(p)
    ...
    private var statusColor: Color {
        guard let p = pressure else { return .secondary }
        if abs(p) < 3 { return .blue }
        return p > 0 ? .green : .red
    }

    private var statusIcon: String {
        guard let p = pressure else { return "gauge.with.dots.needle.50percent" }
    ...
    var body: some View {
        guard let p = pressure else { return AnyView(EmptyView()) }
